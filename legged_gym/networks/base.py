...
class TransformerEncoder(nn.Module):
  ...
  def forward(self, visual_x, detach=False, return_raw_visual_vecs=False):
    ...
    if self.in_channels == 1:
      ...
      # (# Patches ** 2， Batch Shape, Channel)
    if self.in_channels == 4 or self.in_channels == 1:
...
class LocoTransformerEncoder(nn.Module):
  ...
  def forward(self, visual_x, state_x, detach=False, return_raw_visual_vecs=False):
      ...
      if self.in_channels == 1:
          ...
          # print('dvo', depth_visual_out.shape)
          adapool = nn.AdaptiveAvgPool2d((10,10)) 
          output = adapool(depth_visual_out)
          # print('ada', output.shape)
          depth_visual_out = output.reshape(1, 10 * 10) 
          # print('a', depth_visual_out)
          # m = nn.Linear(400,100)
          # if torch.cuda.is_available():
          #       m.cuda()
          # depth_visual_out = m(dvo)
          # print('m', depth_visual_out)
          ...
          #     2, 0, 1
          # )
          # depth_visual_out = depth_visual_out.unsqueeze(0)
          # print('dvo reshape', depth_visual_out.shape)
          # (# Patches ** 2， Batch Shape, Channel)
      ...
      state_out_proj = state_x.unsqueeze(0)
      # print(state_out_proj.shape)

      out_list = [state_out_proj]
      if self.in_channels == 12 or self.in_channels == 16:
          out_list.append(rgb_visual_out)
      if self.in_channels == 4 or self.in_channels == 1 or self.in_channels == 16:
          out_list.append(depth_visual_out)
      visual_out = torch.cat(out_list, dim=1)
      # if return_raw_visual_vecs:
      #     return visual_out, state_out, raw_visual_vecs 
      return visual_out, state_out_proj
